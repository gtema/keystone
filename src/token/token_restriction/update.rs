...
pub async fn update<S: AsRef<str>>(
        ...
        .map_err(|err| db_err(err, "searching for the existing token restriction"))?
    {
        ...
        entry.allow_renew = Set(restriction.allow_renew);
        entry.allow_rescope = Set(restriction.allow_rescope);
        entry.user_id = Set(restriction.user_id);
        entry.project_id = Set(restriction.project_id);
        ...
        let current_roles: BTreeSet<String> = BTreeSet::from_iter(
            DbTokenRestrictionRoleAssociation::find()
                .filter(token_restriction_role_association::Column::RestrictionId.eq(id.as_ref()))
                .select_only()
        ...
        let roles_to_remove: BTreeSet<String> = current_roles
            .iter()
            .filter(|&item| !restriction.role_ids.contains(item))
            .cloned()
            .collect();
        let roles_to_add: BTreeSet<String> = restriction
            .role_ids
            .iter()
            ...
            [
                ...
                Transaction::many(vec![
                    ...
                    Statement::from_sql_and_values(
                        DatabaseBackend::Postgres,
                        r#"UPDATE "token_restriction" SET "user_id" = $1, "allow_renew" = $2, "allow_rescope" = $3, "project_id" = $4 WHERE "token_restriction"."id" = $5 RETURNING "id", "user_id", "allow_renew", "allow_rescope", "project_id""#,
                        [
                            "uid".into(),
                            true.into(),
                            true.into(),
