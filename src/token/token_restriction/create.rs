...
pub async fn create(
    ...
    if !restriction.role_ids.is_empty() {
        DbTokenRestrictionRoleAssociation::insert_many(restriction.role_ids.into_iter().map(
            |rid| token_restriction_role_association::ActiveModel {
    ...
    } else if let Some(roles) = restriction.roles {
        DbTokenRestrictionRoleAssociation::insert_many(roles.into_iter().map(|role| {
        ...
        assert_eq!(
            ...
            [Transaction::many(vec![
                ...
                Statement::from_sql_and_values(
                    ...
                    r#"INSERT INTO "token_restriction" ("id", "user_id", "allow_renew", "allow_rescope", "project_id") VALUES ($1, $2, $3, $4, $5) RETURNING "id", "user_id", "allow_renew", "allow_rescope", "project_id""#,
                    [
                        "1".into(),
                        "uid".into(),
                        true.into(),
